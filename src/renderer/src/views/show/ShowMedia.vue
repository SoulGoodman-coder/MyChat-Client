...
// 关闭窗口时 暂停视频播放（实际上视频组件一直都存在）
const closeWin = ()=>{
    dPlayer.value.pause()
}

const getUrl = (curFile)=>{
    const url =  `http://127.0.0.1:${localServerPort.value}/file?partType=${curFile.partType}&fileId=${curFile.fileId}&fileType=${curFile.fileType}&forceGet=${curFile.forceGet}&${new Date().getTime()}`
    return url
}
