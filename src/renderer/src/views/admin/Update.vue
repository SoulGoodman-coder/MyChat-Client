  ...
  <el-card class="table-data-card">
    <Table :columns="columns" :fetch="loadDataList" :dataSource="tableData" options="tableOptions">
      <template #slotUpdateDesc="{row}">
        <div v-for="(item, num) in row.updateDescArray" :key="num">{{ num+1 }}、{{ item }}</div>
      </template>
      
      <template #slotFileType="{row}">
      ...
      <template #slotOperation="{row}">
        <el-dropdown placement="bottom-end" trigger="click">
          ...
          <template #dropdown>
            ...
            <el-dropdown-item @click="updatePost(row)">
              发布
            </el-dropdown-item>
            <el-dropdown-item @click="del(row)" v-if="row.status == 0">
              删除
            </el-dropdown-item>
          </template>
        </el-dropdown>
